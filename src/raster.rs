...
use geom::Point;
use math::{floor::floorf, ceil::ceilf, sqrt::sqrtf};
...
impl Raster {
    ...
    pub fn draw_line(&mut self, p0: &Point, p1: &Point) {
        ...
        for y in y0..min(self.h, ceilf(p1.y) as usize) {
            let linestart = y * self.w;
            let dy = ((y + 1) as f32).min(p1.y) - (y as f32).max(p0.y);
            let xnext = x + dxdy * dy;
            let d = dy * dir;
            let (x0, x1) = if x < xnext { (x, xnext) } else { (xnext, x) };
            let x0floor = floorf(x0);
    ...
    pub fn draw_quad(&mut self, p0: &Point, p1: &Point, p2: &Point) {
        ...
        let tol = 3.0;
        let n = 1 + floorf(sqrtf(sqrtf(tol * (devx * devx + devy * devy)))) as usize;
        //println!("n = {}", n);
        let mut p = *p0;
